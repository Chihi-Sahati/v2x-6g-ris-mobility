...
class gNB:
    ...
    tx_power_dbm: float = 43.0  # Synced to 3GPP UMa standard EIRP
    load: float = 0.0
...
class V2XSimulator:
    ...
    def step(self, ris_en=True, ris_el=64, ph_b=4, fc=28, bw=400):
        ...
        for v in self.vehicles:
            ...
            # Apply channel-model-based RIS gain (coherent array gain)
            if act == 'optimize_ris' and ris:
                # RIS gain from coherent array: G = (N * phase_alignment)^2 in linear
                N = ris.num_elements
                c = np.abs(np.mean(np.exp(1j * ris.phases)))
                ris_gain_db = 10 * np.log10(max((N * c)**2, 1.0))
                v.sinr_db += ris_gain_db * 0.3  # Realistic fractional RIS contribution
            elif act == 'maintain':
                # Passive tracking gain from existing RIS phases (no active optimization)
                if ris:
                    N = ris.num_elements
                    c = np.abs(np.mean(np.exp(1j * ris.phases)))
                    ris_gain_db = 10 * np.log10(max((N * c)**2, 1.0))
                    v.sinr_db += ris_gain_db * 0.1  # Passive gain
