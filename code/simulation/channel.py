...
class ImperfectCSIConfig:
    """Configuration for imperfect CSI model.

    Manuscript Reference: New section on Imperfect CSI

    Model: h_hat = h + delta_h, where delta_h ~ CN(0, sigma_csi^2)
    Phase noise: theta_actual = theta_ideal + delta_theta, delta_theta ~ N(0, sigma_phase^2)
    """
    csi_error_std: float = 0.1  # sigma_csi: CSI estimation error standard deviation
    phase_noise_std: float = 5.0  # sigma_phase: Phase noise standard deviation in degrees
    pilot_length: int = 200  # Number of pilot symbols for channel estimation
    correlation_time: float = 0.95  # Temporal correlation of CSI errors
...
class ChannelModel:
    ...
    def __init__(self, config: Optional[ChannelConfig] = None):
        ...
        # Imperfect CSI state
        self.imperfect_csi_config = ImperfectCSIConfig()
        self._prev_csi_error: Optional[np.ndarray] = None
        self._prev_phase_noise: Optional[np.ndarray] = None
    ...
    def apply_csi_error(
        self,
        channel: np.ndarray,
        csi_config: Optional[ImperfectCSIConfig] = None
    ) -> np.ndarray:
        """
        Apply imperfect CSI estimation error to a channel vector/matrix.

        Manuscript Reference: Imperfect CSI Model

        Model:
            h_hat = h + delta_h
            where delta_h ~ CN(0, sigma_csi^2) with temporal correlation

        The CSI error is temporally correlated via a first-order Gauss-Markov
        process:
            delta_h(t) = rho * delta_h(t-1) + sqrt(1 - rho^2) * w(t)
            where w(t) ~ CN(0, sigma_csi^2) and rho = correlation_time

        The error variance decreases with pilot length (MMSE bound):
            sigma_eff = sigma_csi / sqrt(pilot_length / 200)

        Args:
            channel: Complex channel coefficients (any shape).
            csi_config: Imperfect CSI configuration (uses default if None).

        Returns:
            Imperfect channel estimate h_hat (same shape as input).
        """
        if csi_config is None:
            csi_config = self.imperfect_csi_config

        shape = channel.shape
        n_elements = channel.size

        # Effective error std scaled by pilot length (MMSE bound)
        # sigma_eff = sigma_csi / sqrt(N_pilot / N_ref)
        sigma_eff = csi_config.csi_error_std / np.sqrt(
            csi_config.pilot_length / 200.0
        )

        # Generate temporally correlated error
        rho = csi_config.correlation_time
        innovation = (
            (np.random.randn(n_elements) + 1j * np.random.randn(n_elements))
            / np.sqrt(2) * sigma_eff
        )

        if self._prev_csi_error is not None and self._prev_csi_error.size == n_elements:
            csi_error = (
                rho * self._prev_csi_error
                + np.sqrt(1 - rho ** 2) * innovation
            )
        else:
            csi_error = innovation

        self._prev_csi_error = csi_error.copy()

        return channel + csi_error.reshape(shape)
    ...
    def apply_phase_noise(
        ...
        """
        Apply phase noise to RIS phase shifts.

        Manuscript Reference: Imperfect CSI Model (Phase Noise)

        Model:
            theta_actual = theta_ideal + delta_theta
            where delta_theta ~ N(0, sigma_phase^2)

        The phase noise is also temporally correlated:
            delta_theta(t) = rho * delta_theta(t-1)
                            + sqrt(1 - rho^2) * N(0, sigma_phase^2)

        Args:
            phases: Ideal RIS phase shifts in radians (any shape).
            csi_config: Imperfect CSI configuration (uses default if None).

        Returns:
            Noisy phase shifts in radians (same shape as input).
        """
        if csi_config is None:
            csi_config = self.imperfect_csi_config

        sigma_phase_rad = np.radians(csi_config.phase_noise_std)
        rho = csi_config.correlation_time
        n_elements = phases.size

        innovation = np.random.randn(n_elements) * sigma_phase_rad

        if self._prev_phase_noise is not None and self._prev_phase_noise.size == n_elements:
            phase_noise = (
                rho * self._prev_phase_noise
                + np.sqrt(1 - rho ** 2) * innovation
            )
        else:
            phase_noise = innovation

        self._prev_phase_noise = phase_noise.copy()

        return phases + phase_noise.reshape(phases.shape)

    def get_csi_error_variance(
        self,
        csi_config: Optional[ImperfectCSIConfig] = None
    ) -> float:
        """
        Get the effective CSI error variance considering pilot length.

        sigma_eff^2 = sigma_csi^2 / (N_pilot / N_ref)

        Args:
            csi_config: Imperfect CSI configuration (uses default if None).

        Returns:
            Effective error variance (linear).
        """
        if csi_config is None:
            csi_config = self.imperfect_csi_config
        sigma_eff = csi_config.csi_error_std / np.sqrt(
