...
class RISOptimizationAgent(BaseV2XAgent):
    ...
    def analyze(self, observation: AgentObservation) -> Dict[str, Any]:
        ...
        # Safe access: if channel_states has wrong shape, use ris_states as fallback
        if csi is None or csi.ndim < 2 or csi.shape[1] < 2:
            # Create synthetic CSI from ris_states and vehicle positions
            num_v = len(observation.vehicle_states)
            num_r = ris_states.shape[0] if ris_states.ndim >= 2 else 1
            csi = np.zeros((num_v, num_r, 2), dtype=np.float32)
            for r in range(min(num_r, 3)):
                csi[:, r, 0] = 0.5 + np.random.RandomState(42+r).rand(num_v) * 0.5
                csi[:, r, 1] = 0.3 + np.random.RandomState(42+r+10).rand(num_v) * 0.7
        ...
        sinr_values = observation.vehicle_states[:, 4] if observation.vehicle_states.shape[1] > 4 else np.full(len(observation.vehicle_states), 10.0)
        worst_vehicle_idx = np.argmin(sinr_values)

        # Compute optimal phase alignment (Equation 8) with bounds checking
        n_channels = min(len(vehicle_ris_channels[worst_vehicle_idx, :]),
                         len(ris_gnb_channels))
        phase_alignment = np.angle(
            vehicle_ris_channels[worst_vehicle_idx, :n_channels] *
            ris_gnb_channels[:n_channels].conj()
        )

        # Pad if needed to match num_elements
        if len(phase_alignment) < self.num_elements:
            phase_alignment = np.pad(phase_alignment, (0, self.num_elements - len(phase_alignment)))
    ...
    def execute(self, action: AgentAction) -> Dict[str, Any]:
        ...
        old_phases = self.current_phases.copy()  # Store before update
        ...
        return {
            ...
            'phase_changes': np.sum(new_phases != old_phases)  # Now correct
        }
